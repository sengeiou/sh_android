...
public class ShotQueueDBMapper extends GenericDBMapper {
  ...
  public ShotQueueEntity fromCursor(Cursor cursor) {
    ...
    if (!cursor.isNull(cursor.getColumnIndex(DatabaseContract.ShotQueueTable.ID_QUEUE))) {
      shotQueueEntity.setIdQueue(
          cursor.getLong(cursor.getColumnIndex(DatabaseContract.ShotQueueTable.ID_QUEUE)));
    } else {
      shotQueueEntity.setIdQueue(null);
    }
    shotQueueEntity.setFailed(
        cursor.getInt(cursor.getColumnIndex(DatabaseContract.ShotQueueTable.FAILED)));
    shotQueueEntity.setImageFile(
        cursor.getString(cursor.getColumnIndex(DatabaseContract.ShotQueueTable.IMAGE_FILE)));

    if (!cursor.isNull(cursor.getColumnIndex(DatabaseContract.ShotQueueTable.ID_SHOT))) {
      shotQueueEntity.setIdShot(
          cursor.getString(cursor.getColumnIndex(DatabaseContract.ShotQueueTable.ID_SHOT)));
    }
    shotQueueEntity.setIdUser(
        cursor.getString(cursor.getColumnIndex(DatabaseContract.ShotQueueTable.ID_USER)));
    shotQueueEntity.setUsername(
        cursor.getString(cursor.getColumnIndex(DatabaseContract.ShotQueueTable.USERNAME)));
    shotQueueEntity.setComment(
        cursor.getString(cursor.getColumnIndex(DatabaseContract.ShotQueueTable.COMMENT)));
    shotQueueEntity.setImage(
        cursor.getString(cursor.getColumnIndex(DatabaseContract.ShotQueueTable.IMAGE)));
    shotQueueEntity.setStreamTitle(
        cursor.getString(cursor.getColumnIndex(DatabaseContract.ShotQueueTable.STREAM_TITLE)));
    shotQueueEntity.setIdStream(
        cursor.getString(cursor.getColumnIndex(DatabaseContract.ShotQueueTable.ID_STREAM)));
    shotQueueEntity.setType(
        cursor.getString(cursor.getColumnIndex(DatabaseContract.ShotQueueTable.TYPE)));

    if (!cursor.isNull(cursor.getColumnIndex(DatabaseContract.ShotQueueTable.ID_SHOT_PARENT))) {
      shotQueueEntity.setIdShotParent(
          cursor.getString(cursor.getColumnIndex(DatabaseContract.ShotQueueTable.ID_SHOT_PARENT)));
    }
    if (!cursor.isNull(cursor.getColumnIndex(DatabaseContract.ShotQueueTable.ID_USER_PARENT))) {
      shotQueueEntity.setIdUserParent(
          cursor.getString(cursor.getColumnIndex(DatabaseContract.ShotQueueTable.ID_USER_PARENT)));
    }
    if (!cursor.isNull(cursor.getColumnIndex(DatabaseContract.ShotQueueTable.USERNAME_PARENT))) {
      shotQueueEntity.setUserNameParent(
