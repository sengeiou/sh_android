...
public class UserManager extends AbstractManager {
  ...
  @Inject public UserManager(SQLiteOpenHelper openHelper, UserEntityDBMapper userMapper) {
    super(openHelper);
  ...
  public List<UserEntity> getUsersByIds(List<String> usersIds) {
    ...
    String whereClause =
        DatabaseContract.UserTable.ID + " IN (" + createListPlaceholders(userIdsSize) + ")";
  ...
  public List<UserEntity> searchUsers(String searchString) {
    String stringToSearch =
        Normalizer.normalize(searchString, Normalizer.Form.NFD).replaceAll("[^\\p{ASCII}]", "");
  ...
  @Nullable private UserEntity readUser(String whereClause, String[] whereArguments) {
    UserEntity resUser = null;
    Cursor c = getReadableDatabase().query(DatabaseContract.UserTable.TABLE,
        DatabaseContract.UserTable.PROJECTION, whereClause, whereArguments, null, null, null, null);
    if (c.getCount() > 0) {
      c.moveToFirst();
  ...
  @NonNull private List<UserEntity> readUsers(String whereClause, String[] whereArguments) {
    Cursor queryResults = getReadableDatabase().query(DatabaseContract.UserTable.TABLE,
        DatabaseContract.UserTable.PROJECTION, whereClause, whereArguments, null, null,
  ...
  public List<UserEntity> getRelatedUsersByIdStream(String idStream, String idUser) {
    String whereClause = DatabaseContract.UserTable.ID
        + "<> ? AND "
        + DatabaseContract.UserTable.ID_WATCHING_STREAM
        + " == ?";
    String[] whereArguments = new String[] { idUser, idStream};
