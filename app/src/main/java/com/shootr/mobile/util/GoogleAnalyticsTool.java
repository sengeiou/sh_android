...
public class GoogleAnalyticsTool implements AnalyticsTool {
    ...
    public void init(Context context) {
        ...
        tracker = analytics.newTracker(context.getString(com.shootr.mobile.R.string.google_analytics_tracking_id));
    ...
    public synchronized Tracker getTracker(Context context, TrackerName appTracker) {
        if (!mTrackers.containsKey(appTracker)) {
            GoogleAnalytics analytics = GoogleAnalytics.getInstance(context);
            Tracker t = analytics.newTracker(com.shootr.mobile.R.xml.navigation_tracker);
