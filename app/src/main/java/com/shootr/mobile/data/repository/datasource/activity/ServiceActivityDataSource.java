...
public class ServiceActivityDataSource implements ActivityDataSource {
  ...
  @Inject public ServiceActivityDataSource(ActivityApiService activityApiService,
      ...
      ShotApiEntityMapper shotApiEntityMapper, SessionRepository sessionRepository) {
  ...
  @Override public List<ActivityEntity> getActivityTimeline(ActivityTimelineParameters parameters,
      String locale) {
    try {
      List<ActivityApiEntity> activities =
          activityApiService.getActivityTimeline(parameters.getIncludedTypes(),
              parameters.getStreamTypes(), parameters.getShotTypes(), parameters.getLimit(),
              parameters.getSinceDate(), parameters.getMaxDate(), locale,
      ...
      for (ActivityApiEntity activity : activities) {
        ...
        if (hasEmbedShot) {
          localShotDatasource.putShot(shot, sessionRepository.getCurrentUserId());
        }
      }
