...
@Singleton public class ShotNotificationManager {
  ...
  @Inject public ShotNotificationManager(Application context,
  ...
  public void sendNewShotNotification(ShotNotification shot, Boolean areShotTypesKnown, Boolean isInApp) {
    shotsCurrentlyNotified.put(shot.getIdShot(), shot);

    CommonNotification notification;
    if (shotsCurrentlyNotified.size() > 1) {
      notification =
          buildMultipleShotNotification(new ArrayList<>(shotsCurrentlyNotified.values()), isInApp);
    } else {
      notification = buildSingleShotNotification(shot, areShotTypesKnown, isInApp);
    }
