...
function initCacheMonitor(maxBytesLocalHeap, maxBytesLocalDisk,
	...
	maxLocalHeap = parseInt(maxBytesLocalHeap);
	maxLocalDisk = parseInt(maxBytesLocalDisk);
	
	cacheHAHeapHistoryChart = ChartMonitor(cacheHAHeapHistory);
	cacheHAHeapChart = ChartMonitor(cacheHAHeap);

	cacheHADiskHistoryChart = ChartMonitor(cacheHADiskHistory);
	cacheHADiskChart = ChartMonitor(cacheHADisk);

	cacheHeapHistoryChart = ChartMonitor(cacheHeapHistory);
	cacheHeapChart = ChartMonitor(cacheHeap);

	cacheDiskHistoryChart = ChartMonitor(cacheDiskHistory);
	cacheDiskChart = ChartMonitor(cacheDisk);
}
...
function monitorRequestWindow(cacheMonitorWindow, isRefresh) {
	...
	/*
	var ret = [
	       jsonResponse["data"]["TotalDiskHit"],
	       jsonResponse["data"]["TotalDiskMiss"],
	       jsonResponse["data"]["TotalDiskSpace"],
	       
	       jsonResponse["data"]["TotalHeapHit"],
	       jsonResponse["data"]["TotalHeapMiss"],
	       jsonResponse["data"]["TotalHeapSpace"]
	       ];
	 */
