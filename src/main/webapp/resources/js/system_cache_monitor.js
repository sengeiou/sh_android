 ...
 */
function applyChange(periodSelector, segmentSelector, datetimeFrom, datetimeTo, applyTimeWindow) {
	...
	if (to != null && from != null) {
		...
		monitorRequestWindow(cacheMonitorWindow, false);
 ...
 */
function initSystemCacheMonitor(cacheManager, cache, maxBytesLocalHeap, maxBytesLocalDisk,
...
)
{
	...
	applyTimeWindow.on("click", function (e) {
		...
		applyChange(period, segment, datetimeFrom, datetimeTo, applyTimeWindow);
	});

	initCacheMonitor(maxBytesLocalHeap, maxBytesLocalDisk,
			cacheHAHeapHistory, cacheHAHeap, 
			cacheHADiskHistory, cacheHADisk, 
			cacheHeapHistory, cacheHeap, 
			cacheDiskHistory, cacheDisk);
	...
	App.destructionFunction = destroySystemCacheMonitor;
 ...
 */
function destroySystemCacheMonitor() {
