...
public class InsertShot implements GenericServiceHook {
    ...
    public <T> HookMethodOutput requestBegin(T connection, AccessPolicy accessPolicy, Generic generic) {
        ...
        for (int i = 0; i < numberOfOperations; i++) {
            if (isOperationMod(generic.getOperations().get(i).getMetadata().getOperation())) {
                int numberOfDataItems = generic.getOperations().get(i).getData().size();
                for (int j = 0; j < numberOfDataItems; j++) {

                    for (Map.Entry<String, Object> entry : generic.getOperations().get(i).getData().get(j).getAttributes().entrySet()) {
