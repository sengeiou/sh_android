...
public class ChangePasswordValidator {
    ...
    private void validateNewPasswordIsDifferentUsername(String newPassword, String username) {
        if(newPassword != null && newPassword.equals(username)) {
            addError(com.shootr.mobile.domain.exception.ShootrError.ERROR_CODE_NEW_PASSWORD_EQUALS_USERNAME, FIELD_NEW_PASSWORD_AGAIN);
        }
    }
    ...
    private void validatePassWordHasNotInvalidCharacters(String password, int field) {
        if (password != null) {
            ...
            if (!hasPasswordPattern) {
                addError(com.shootr.mobile.domain.exception.ShootrError.ERROR_CODE_REGISTRATION_PASSWORD_INVALID_CHARACTERS, field);
            }
        }
    }
    ...
    private void validateNewPasswordIsDifferentFromCurrentPassword(String username, String password) {
        if(username!=null && username.equals(password)){
            addError(com.shootr.mobile.domain.exception.ShootrError.ERROR_NEW_PASSWORD_EQUALS_CURRENT_PASSWORD, FIELD_NEW_PASSWORD_AGAIN);
        }
    }
    ...
    private void validateNewPasswordEqualsNewPasswordAgain(String newPassword, String newPasswordAgain) {
        if(newPassword != null && newPasswordAgain!=null && !newPassword.equals(newPasswordAgain)){
            addError(com.shootr.mobile.domain.exception.ShootrError.ERROR_NEW_PASSWORD_NOT_EQUALS_NEW_PASSWORD_AGAIN, FIELD_NEW_PASSWORD_AGAIN);
        }
    }
